 ...
 */
export async function tryRestart(
  ...
  if (attempts >= MAX_RESTART_ATTEMPTS) {
    console.error(
      `  [fixer] ${config.name}: max restart attempts (${MAX_RESTART_ATTEMPTS}) reached. Giving up.`
    );
    return false;
  }
  ...
  console.log(
    `  [fixer] ${config.name}: restarting in ${delay}ms (attempt ${attempts + 1}/${MAX_RESTART_ATTEMPTS})...`
  );
 ...
 */
export async function withErrorNormalization<T>(
  serverName: string,
  operation: string,
  fn: () => Promise<T>
): Promise<T> {
...
function normalizeMcpError(
  ...
  if (lower.includes("enoent") || lower.includes("spawn")) {
    ...
    return (
      `Server "${server}" failed to start: command not found${cmdHint ? ` (${cmdHint})` : ""}. ` +
  ...
  if (
    ...
    return (
      `Server "${server}" crashed during ${operation}. ` +
  ...
  if (lower.includes("timeout") || lower.includes("etimedout")) {
    return (
      `Server "${server}" timed out during ${operation}. ` +
  ...
  if (lower.includes("json") && (lower.includes("parse") || lower.includes("unexpected"))) {
    return (
      `Server "${server}" returned invalid JSON during ${operation}. ` +
...
function extractCommand(msg: string): string | null {
  ...
  for (const tool of tools) {
    ...
    if (!tool.description || tool.description.trim() === "") {
      issues.push({
        ...
        tool: tool.name,
        issue:
          "Missing description — LLMs will struggle to select this tool correctly",
        severity: "warn",
      });
    }
 ...
 */
export function logSchemaIssues(issues: SchemaIssue[]): void {
  ...
  for (const issue of issues) {
    ...
    console.log(
      `    ${icon} ${issue.server}/${issue.tool}: ${issue.issue}`
    );
  }
  console.log("");
}
